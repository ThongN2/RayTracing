...
public:
    ...
    //
    ofColor diffuseColor = ofColor::red;    // default colors - can be changed.
    ofColor specularColor = ofColor::lightGray;
    virtual ofColor diffuseTexture(ofImage* diffuseImage, glm::vec3 pt) {
        return diffuseColor;
    }
    
    virtual ofColor specTexture(ofImage* specImage, glm::vec3 pt) {
...
//Light Class with intensity = 1, and radius = 0.1
//
class Light : public SceneObject {
public:
    Light(glm::vec3 p, float intensity, ofColor diffuse = ofColor::lightGray) {position = p; this->intensity = intensity ; diffuseColor = diffuse ;}
    Light(){}
    void draw()
    {
        ofDrawSphere(position, radius);
...
glm::vec3 getNormal(const glm::vec3 &p) { return this->normal; }
    glm::vec2 getXY(const glm::vec3 &p1) {
  
        return  glm::vec2(p1.x-0,p1.z-0);
...
float width = 20;
float height = 20;
    ofColor specTexture(ofImage* specImage, glm::vec3 pt);
    ofColor diffuseTexture(ofImage* diffuseImage, glm::vec3 pt);
};
...
class ofApp : public ofBaseApp{
    ...
    public:
        ...
        void rayTrace();
    bool Shadow(Ray &ray);
    ofColor lambert(const glm::vec3 &p, const glm::vec3 &norm,const float lightIntensity,const ofColor diffuse);
    ofColor ambient(const ofColor diffuse, float percentage);
    ofColor phong(const glm::vec3 &p, const glm::vec3 &norm,const float lightIntensity,
                         const ofColor diffuse, const ofColor specular, float power);
    ofColor texture_lookup(ofImage i, float u, float v);
    ofColor shadeSurface(const glm::vec3 p,const ofImage i);
        bool bHide = true;
        ...
        RenderCam renderCam;
        ofImage image;
    ofImage diffuseImage1;
    ofImage diffuseImage2;
    ofImage specImage1;
    ofImage specImage2;
